...
import Link from "next/link"; // Usamos Link solo para el Logo
...
export default function Navbar() {
  ...
  useEffect(() => {
    AOS.init({
      ...
      once: true,     // Si es true, anima solo la primera vez que bajas (más elegante)
    });
  }, []);
    ...
    <>
      <nav className="fixed w-full z-50 flex justify-between items-center px-8 py-4 bg-black/90 backdrop-blur-md border-b border-white/10">
        ...
        <div className="hidden md:flex gap-8 text-sm font-medium text-gray-300">
          {/* CAMBIO CLAVE: Usamos 'a' en lugar de 'Link' para el scroll */}
